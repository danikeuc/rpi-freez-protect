...
if [ ! -f "$helper_source" ] || [ ! -f "$sudoers_source" ] || \
   [ ! -f "$sshd_policy_source" ] || [ ! -f "$client_source" ]; then
...
validate_service_account() {
  ...
  if ! { [ "$(id -u "$service_account")" -ne 0 ] &&
         [ "$(id -g "$service_account")" -ne 0 ] &&
...
validate_commission_account() {
  ...
  for prohibited_group in gpio dialout; do
  ...
  if ! { [ "$(id -u "$commission_account")" -ne 0 ] &&
         [ "$(id -g "$commission_account")" -ne 0 ] &&
